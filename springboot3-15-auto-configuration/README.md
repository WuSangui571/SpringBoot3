...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean class="org.mybatis.spring.mapper.MapperScannerConfigurer">
        <property name="basePackage" value="com.sangui.mapper"/>
    ...
    <!-- 扫描 service 层的包 -->
    <context:component-scan base-package="com.sangui.service"/>
...
springboot315AutoConfigurationApplication
...
也就是说，引入了`web启动器`后，ioc容器中增加了`104`个bean对象（**<font style="color:#DF2A3F;">加入了104个组件</font>**）。这`104`个bean对象都是为web开发而准备的，例如我们常见的：
...
[SpringBoot关于默认配置的官方文档](https://docs.spring.io/spring-boot/appendix/application-properties/index.html)
...
任何启动器都会关联引入这样一个启动器：`spring-boot-starter`，它是spr ingboot框架最核心的启动器。
...
3. 核心启动器导入之后，关联导入了一个jar包：`spring-boot-autoconfigure`。
   1. 注意：这个jar包中存放的是springboot框架**<font style="color:#DF2A3F;">官方支持的自动配置类</font>**。

​	  2. 官方支持的自动配置类有多少个呢，在`\mvn_resp\org\springframework\boot\spring-boot-autoconfigure\3.4.4\spring-boot-autoconfigure-3.4.4.jar\META-INF\spring\org.springframework.boot.autoconfigure.AutoConfiguration.imports`中显示153行

​	得知`springboot3.4.5`这个版本共`153个自动配置类。自动配置类的命名规则是`XxxxAutoConfiguration`。
...
4. 添加断点，跟踪`AutoConfigurationImportSelector`源码
   ...
   AutoConfigurationImportSelector类的protected AutoConfigurationEntry getAutoConfigurationEntry(AnnotationMetadata annotationMetadata)方法的这一行：

   ```java
   List<String> configurations = getCandidateConfigurations(annotationMetadata, attributes);
   ```

   通过跟踪得知，这`153`个自动配置类的**类名**都会被加载到IoC容器中。**<font style="color:#DF2A3F;">注意：加载了153，并不是153个全部生效</font>**。

5. 这`153`个自动配置类底层是怎么查找的？

   通过以上源码跟踪，得知，是从以下位置加载的：\mvn_resp\org\springframework\boot\spring-boot-autoconfigure\3.4.4\spring-boot-autoconfigure-3.4.4.jar\META-INF\spring\org.springframework.boot.autoconfigure.AutoConfiguration.imports

6. 最终哪些自动配置类生效了？

   最先获取到`153`个，经过上图的一层一层的过滤（**条件注解**），**<font style="color:#DF2A3F;">最终筛选了</font>**`14个自动配置类，为什么这么少，因为你只引入了web starte`**<font style="color:#DF2A3F;">。这14个配置就是做web开发需要的最少配置</font>**。
...
通过以上源码得知，`DispatcherServletConfiguration`组件的配置信息来源于`WebMvcProperties`属性类。源码如下：
